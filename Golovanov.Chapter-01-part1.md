...
$$ r(t) = p + r \cos t i_x + r \sin t i_y + \frac{ht}{2\pi} i_z. $$
(1.2.3)
...
$$ r(t) = a_0 + a_1(t - t_0) + a_2(t - t_0)(t - t_1) + \ldots + a_n(t - t_0)(t - t_1)\ldots(t - t_{n-1}). $$
(1.3.5)
...
$$ r_i^{(k)} = (1-t)r_i^{(k-1)} + tr_{i+1}^{(k-1)}, \quad i + k \leq n. $$
(1.4.5)
...
$$M_i^m(t) = \sigma_m[t_i, t_{i+1}, \ldots, t_{i+m+1}].$$
(1.8.2)
...
$$ = -\frac{m}{t_{i+m+1} - t_{i+1}} N_{i+1}^{m-1}(t) + \frac{m}{t_{i+m} - t_i} N_i^{m-1}(t). $$
(1.8.15)
...
$$ \int_{-\infty}^{\infty} M_i^m(t) dt = \int_{t_{\min}}^{t_{\max}} M_i^m(t) dt = \frac{1}{m+1}, $$
(1.8.16)
